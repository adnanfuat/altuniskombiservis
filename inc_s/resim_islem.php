...
//******************************* (s) Ya Guncelleme Yapiliyor ya da Yeni Kayit Ekleniyor *******************************************//
		   ...
		   if  ((isset($_FILES["resim"]) && strlen($_FILES["resim"]["name"])!=0 &&  ($_FILES["resim"]["type"]=='image/gif' || $_FILES["resim"]["type"]=='image/jpg' || $_FILES["resim"]["type"]=='image/pjpeg' || $_FILES["resim"]["type"]=='image/jpeg' || $_FILES["resim"]["type"]=='image/png')  && $_FILES["resim"]["size"]<$max_file_size ) && ($insert=='1'))																															
		  ...
		  else if  ((isset($_FILES["resim"]) && strlen($_FILES["resim"]["name"])!=0  &&  ($_FILES["resim"]["type"]=='image/gif' || $_FILES["resim"]["type"]=='image/jpg' || $_FILES["resim"]["type"]=='image/pjpeg' || $_FILES["resim"]["type"]=='image/jpeg' || $_FILES["resim"]["type"]=='image/png')  && $_FILES["resim"]["size"]<$max_file_size)&&($insert=='0'))																															
		   ...
		   if  (isset($_FILES["resim"]) && strlen($_FILES["resim"]["name"])!=0)	// Resim Gelmis mi?																															
	             {
						   ...
						   if ($dosya_size>$max_file_size)	  {	$err='boyut'; }																   
						           ...
						           {				 
										...
										if ($dosya_tip=='image/jpeg' ||  $dosya_tip=='image/jpg' ||  $dosya_tip=='image/pjpeg' || $dosya_tip=='image/png') // Eger jpeg ya da bmp ise islem yap
											  {  						
												 if ($dosya_tip=='image/gif') {$uzanti='gif'; } else if ($dosya_tip=='image/png') {$uzanti='png'; } else {$uzanti='jpg';}
													...
													if ($insert==0) {$numara=$_POST['gizli'];} //print "numara deger gizliden alindi <br> numara: ".$numara;} 
														...
														// (s)  Daha Önce Ayni Adda Bir Resim Varsa Sil 
															  ...
															  if (file_exists($resim_dizini."resim".$numara.".png"))    {  chmod($resim_dizini."resim".$numara.".png",0777); unlink($resim_dizini."resim".$numara.".png");}
