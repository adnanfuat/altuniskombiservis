...
if (isset($_SESSION['yonet']))
   {
		...
		if  (isset($_FILES["video"]) && strlen($_FILES["video"]["name"])!=0)// && $_FILES["video"]["type"]=="application/octet-stream"
    	    {
			   ...
			   if ($dosya_size>$max_file_size)
				  ...
				  {				 
						...
						$video_ekle="Update $tablo_ismi  Set video_adres='$video', ad='$ad', kaynak='$kaynak', durum='$durum', dil='$dil', aciklama='$aciklama', htaccess_etiket='$htaccess_etiket', puan='$puan' where numara=$numara";
				   ...
				   if ($dosya_size>$max_file_size)
					  ...
					  {				 
						   if   (count($recordset)<>0)
							...
							$resim_ekle="Update $tablo_ismi  Set resim_adres='$dosya2',ad='$ad',kaynak='$kaynak',durum='$durum',dil='$dil',puan='$puan' where numara=$numara";
		...
		if (isset($err))
		   ...
		   {
				$video_ekle="Update $tablo_ismi  Set ad='$ad', kaynak='$kaynak', durum='$durum', dil='$dil', htaccess_etiket='$htaccess_etiket', aciklama='$aciklama',puan='$puan' where numara=$numara";
				@$baglanti->query($video_ekle)->fetchAll(PDO::FETCH_ASSOC);
