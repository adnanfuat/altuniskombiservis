...
if  (isset($_SESSION['yonet']))
	{
		...
		if (isset($_GET["item"])) 
	 	   { 
			  ...
			  if (file_exists($thumb_resim_dizini2.$resim_adres) && $thumb_resim_dizini2.$resim_adres<>$thumb_resim_dizini2) 
			  	 { 
				 	 @chmod($thumb_resim_dizini2.$resim_adres,0755); @unlink($thumb_resim_dizini2.$resim_adres);
				 }
    ...
    if  (isset($_POST["sil"]))
		{     
			 ...
			 $recordset_bul="Select * from  $tablo_ismi order by numara asc limit $ilkkayit,$bir_sayfadaki_toplam_kayit_sayisi";
			 $recordset=@$baglanti->query($recordset_bul)->fetchAll(PDO::FETCH_ASSOC);
