...
if (isset($_SESSION['yonet']))
   {
		...
		// $syonetim=$_SESSION["siteyonetici"];
		 //*******************************************************************************************************************
		 if  (isset($_GET["item"])) // (s) Resim Sil linkiyle gelindiyse ilgili kaydin resmi ve veritabanindaki resim bilgisi siliniyor..
			 { 
				  $item=$_GET["item"];
				  $sayfa_no=$_GET["sayfa"];
				  ...
				  $resim_sil=@$baglanti->query("Update urun_altkategori set resim_adres='' where numara='$item'");
				 ...
				 if (isset($_POST["tasino"]))
				 	{
						...
						if  (isset($_SESSION[$tablo_ismi.'tasi_altkategori']))
							{
								unset($HTTP_SESSIO_VARS[$tablo_ismi.'tasi_altkategori']);
							}
	   ...
	   if  (isset($_POST["sil"]) || isset($_POST["sil2"]))
		   {     
				 ...
				 if (isset($_POST["silinecekler_numara"]))
					...
					$sql_cumlesi="select numara from $tablo_ismi  where ustkategori_no='$ustkategori_no'";
