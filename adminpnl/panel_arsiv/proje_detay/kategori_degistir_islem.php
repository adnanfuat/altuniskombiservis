...
if (isset($_SESSION['yonet']))
   {
		...
		$sql="Select numara from  $tablo_ismi where altkategori_no=$altkategori_no and yonetim='$syonetim'";
		...
		if  (isset($_POST["guncelle"]))
			{
				if (isset($_SESSION['tasi_ekart']))
					...
					} // (f) Guncelle butonuna basilarak gelinmisse
				$ust=$_POST['menu_ust_kategori']; // Firma icin secilen yeni ust kategori numarasi
				$alt=$_POST["menu_alt_kategori"]; 
				 ...
				 header("Location:kontrol.php?alt_kategori=$alt&ust_kategori=$ust"); echo "aa";
			}
		////******************************************************************************************************************
		if (isset($_POST["menu_ust_kategori"]))
			{
				...
				$altsektor=@$baglanti->query("Select numara from  urun_altkategori where ustkategori_no=$menu_ust_kategori and yonetim='$syonetim'")->fetchAll(PDO::FETCH_ASSOC);
