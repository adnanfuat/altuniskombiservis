...
if (isset($_SESSION['yonet']))
   {
		...
		if  ($videomu==1 && isset($_FILES["video"]) && strlen($_FILES["video"]["name"])!=0) //  && $_FILES["video"]["type"]=="application/octet-stream"
	        {
			   ...
			   if ($dosya_size>$max_file_size || $dosya_size2>$max_file_size)
	              ...
	              {		
			            $ekle="Insert into $tablo_ismi (ad,kaynak,aktif,durum,kategori,puan,videomu) Values ('$n_ad','$n_kaynak','1','$n_durum','$kategori','$n_puan','$videomu')";
		...
		else if ($videomu==1 && strlen($n_kaynak)>0)
			{
				$ekle="Insert into $tablo_ismi (ad,kaynak,aktif,durum,kategori,puan,videomu) Values ('$n_ad','$n_kaynak','1','$n_durum','$kategori','$n_puan','$videomu')";
				@$baglanti->query($ekle)->fetchAll(PDO::FETCH_ASSOC);		
			}
		else if ($videomu==0 &&  isset($_FILES["resim"]) && strlen($_FILES["resim"]["name"])!=0)
			{
				if  (isset($_FILES["resim"]) && strlen($_FILES["resim"]["name"])!=0 &&  ($_FILES["resim"]["type"]=='image/jpeg' ||  $_FILES["resim"]["type"]=='image/jpg' ||  $_FILES["resim"]["type"]=='image/pjpeg'))
					{
						   ...
						   if ($dosya_size>$max_file_size)
							  ...
							  {				 
									$ekle="Insert into $tablo_ismi (ad,aktif,kategori,puan,videomu) Values ('$n_ad','1','$kategori','$n_puan','$videomu')";
									@$baglanti->query($ekle)->fetchAll(PDO::FETCH_ASSOC);		
