...
if (isset($_SESSION['yonet']))
   {
		...
		if  (isset($_FILES["dosya"]) && strlen($_FILES["dosya"]["name"])!=0)
			{
			   ...
			   if ($dosya_size>$max_file_size)
				  ...
				  {				 
						...
						$dosya_ismi="dokuman".$numara.$uzanti;
		...
		if (isset($err))
		   ...
		   {
				$ekle="Update $tablo_ismi set ad='$ad', dil='$dil' where numara=$numara"; // and dil='$dil' 
				
				//echo "Update $tablo_ismi set ad='$ad'  where numara=$numara";
				@$baglanti->query($ekle)->fetchAll(PDO::FETCH_ASSOC);
